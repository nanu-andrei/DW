...
export default function AssetDetailPage() {
  ...
  const loadDetails = async () => {
    ...
    setError(null);
    try {
      const data = (await fetchAssetDetails(assetId)) as Asset[];
      setVersions(data);
    } catch (e) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-terminal-cyan mt-1">{assetId}</p>
      </div>
        ...
        <div className="bg-terminal-surface border border-terminal-green/30 rounded p-4">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div className="col-span-2">
              ...
              <span className="text-terminal-text">{latest.description}</span>
            </div>
          </div>
            ...
            <div className="mt-3">
              ...
              <div className="grid grid-cols-3 gap-2 mt-2">
                {Object.entries(latest.attributes).map(([k, v]) => (
                  <div
                    key={k}
                    className="text-xs bg-terminal-bg px-2 py-1 rounded border border-terminal-border"
                  >
          ...
          {versions.map((v, i) => (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                {v.deleted && (
                  <span className="px-1.5 py-0.5 bg-terminal-red/20 text-terminal-red rounded text-[10px]">
                    DELETED
                  </span>
                )}
              </div>
